...
import type { AnomalyDetectionResult } from '@/types/ai-features'
...
export function AnomalyCard({ result }: { result: AnomalyDetectionResult }) {
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1.5">
          <div className="h-1.5 w-12 rounded-full bg-muted overflow-hidden">
            <div
              className={`h-full rounded-full ${RISK_COLORS[result.overallRisk]}`}
      ...
      {!hasAnomalies ? (
        ...
        <div className="space-y-2">
          {result.anomalies.map((a) => (
            <div key={a.id} className={`rounded-lg border p-2.5 space-y-1 ${SEVERITY_STYLES[a.severity]}`}>
              <div className="flex items-center gap-2">
                <span className={`rounded-full border px-1.5 py-0.5 text-[9px] font-bold ${SEVERITY_STYLES[a.severity]}`}>
              ...
              <div className="flex items-center justify-between pt-0.5">
                <span className="text-[10px]">{a.temperatureRange.min}°C – {a.temperatureRange.max}°C</span>
